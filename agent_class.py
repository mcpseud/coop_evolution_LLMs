...
class Agent:
    ...
    def communicate(self, context: Dict, previous_messages: List[Dict]) -> Optional[str]:
        """Generate a communication message to send to opponent"""
    ...
    def make_move(self, context: Dict, messages: List[Dict]) -> str:
        """Decide on a move in the game"""
        ...
        # Extract the actual move from response
        move = self._extract_move(response, context['scenario']['game_type'])
    ...
    def update_memory_after_pairing(self, opponent_id: str, pairing_history: List[Dict]):
        """Update memory about an opponent after a pairing"""
    ...
    def generate_gossip(self, about_agent: str, to_agent: str) -> Optional[str]:
    ...
    def receive_gossip(self, from_agent: str, about_agent: str, gossip: str):
        """Process received gossip and update memories"""
    ...
    def _extract_move(self, response: str, game_type: str) -> str:
        """Extract the actual move from LLM response"""
        response_lower = response.lower().strip()
        
        # Map common responses to standard moves based on game type
        if game_type == "prisoners_dilemma":
            if any(word in response_lower for word in ['cooperate', 'collaborate', 'trust']):
                return 'cooperate'
            elif any(word in response_lower for word in ['defect', 'betray', 'compete']):
                return 'defect'
        
        elif game_type == "stag_hunt":
            if any(word in response_lower for word in ['stag', 'collaborate', 'team']):
                return 'stag'
            elif any(word in response_lower for word in ['hare', 'solo', 'individual']):
                return 'hare'
        
        elif game_type == "hawk_dove":
            if any(word in response_lower for word in ['hawk', 'aggressive', 'fight']):
                return 'hawk'
            elif any(word in response_lower for word in ['dove', 'peaceful', 'yield']):
                return 'dove'
        ...
        # If no clear match, return the first word as the move
        first_word = response.split()[0] if response.split() else 'cooperate'
